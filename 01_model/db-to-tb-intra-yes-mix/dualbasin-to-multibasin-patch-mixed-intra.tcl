#!/usr/bin/tclsh
# creates a patch of basin 3,4, etc multicontacts
# input: DB grotop file
# param: LIST_DOMAINS, LIST_DOM_LIKE_BASIN_1/2, LIST_DOM_MIX
# param: BASIN_NEW_NUM: the number to give to the basin
# param: STR_TRIPBAS: name of output patch
# param: PDB_NAME_BASIN_1/2: pdb files of basin 1/2
# output: a patch of basin 3 (4 etc) of multicontact entries for an intermediate basin, where contacts of LIST_DOM_MIX are mixed according to distance from domains
##############################################################################################################
set FILE_NAME_IN GO_DoME_DB_4akea_1akea_0p6dc_2p6cr.grotop
set LIST_DOMAINS {{{1 6} {14 29} {60 114} {175 214}} {{7 13} {115 120} {159 174}} {{30 59}} {{121 158}}}
#set LIST_DOM_LIKE_BASIN_1 {{1 3} {2 3} {3 3} {3 4}} 
#set LIST_DOM_LIKE_BASIN_2 {{1 2} {1 4} {2 4} {2 2} {4 4}}
set LIST_DOM_LIKE_BASIN_1 {{1 2} {1 4} {2 4} {2 2} {4 4} {2 3} {3 4}}
set LIST_DOM_LIKE_BASIN_2 {{1 3} {3 3}} 
set LIST_DOM_MIX {{1 1}}
set BASIN_NEW_NUM 3
set STR_TRIPBAS nmp_intra_mix
set PDB_NAME_BASIN_1 GO_4akea.pdb
set PDB_NAME_BASIN_2 GO_1akea.pdb
set DIST_MIN_CUTOFF_START 100
set SCRIPT_NAME dualbasin-to-multibasin-patch-mixed-intra
###############################################################################################################
# i/o
#####
...
###############################################################################################################
# expand domain range definitions to all residues
set list_list_dom_resid {}
foreach list_pair_domain $LIST_DOMAINS {
	set list_dom_resid {}
	foreach pair_range $list_pair_domain {
		set res_start [lindex $pair_range 0]
		set res_end [lindex $pair_range 1]
		set list_range_expand_resid [proc_get_list_int_between_beg_end_yes_inc $res_start $res_end]
		lappend list_dom_resid $list_range_expand_resid
	}
	set list_dom_resid [join $list_dom_resid]
	lappend list_list_dom_resid $list_dom_resid
}
puts "set list_list_dom_resid"

# read multicontact entries from original DB file
set file_in [open $FILE_NAME_IN r]
set is_read_multicontact 0
set list_res1 {}
set list_res2 {}
set list_basin {}
set list_dist {}
...
set list_line {}
while {[gets $file_in line]>=0} {
	if {$is_read_multicontact==1} {
		if {[llength $line]==6} {
			# if multicontact
			lappend list_res1 [lindex $line 0]
			lappend list_res2 [lindex $line 1]
			lappend list_basin [lindex $line 3]
			lappend list_dist [lindex $line 4]
			lappend list_energy [lindex $line 5]
			lappend list_line $line
			set list_pair [lrange $line 0 1]
			set list_pair [lsort -integer $list_pair]
			lappend list_pair_sort $list_pair
		} else {
			set is_read_multicontact 0
			break
		}
	...
	if {[string match *\\\[* $line]==1} {
		if {[string match *multicontact* $line]==1} {
			set is_read_multicontact 1
		}
	}
}
close $file_in
puts "set for multicontact list: list_res1, list_res2, list_basin, list_dist, list_energy"

# assign domain pair for each contact
set list_dom_res1 {}
set list_dom_res2 {}
foreach res1 $list_res1 res2 $list_res2 {
	set ind_dom_res1 [proc_lsearch_sublist $list_list_dom_resid $res1]
	set ind_dom_res2 [proc_lsearch_sublist $list_list_dom_resid $res2]
	set dom_res1 [expr $ind_dom_res1+1]
	set dom_res2 [expr $ind_dom_res2+1]
	lappend list_dom_res1 $dom_res1
	lappend list_dom_res2 $dom_res2
}
puts "set list_pair_dom1_dom2"

# determine which contacts from each basin to add to intermediate (1=add, 0=do not add, 2=undecided, mixed)
# at first, only deal with non-mixed contacts
set list_is_intermediate {}
foreach basin $list_basin dom_res1 $list_dom_res1 dom_res2 $list_dom_res2 {
	set pair_dom_res [list $dom_res1 $dom_res2]
	set pair_dom_res [lsort -integer $pair_dom_res]
	set is_intermediate -1
	if {$basin==1} {
		if {[lsearch $LIST_DOM_LIKE_BASIN_1 $pair_dom_res]>-1} {
			set is_intermediate 1
		} elseif {[lsearch $LIST_DOM_LIKE_BASIN_2 $pair_dom_res]>-1} {
			set is_intermediate 0
		} else {
			set is_intermediate 2
		}
	} else {
		if {[lsearch $LIST_DOM_LIKE_BASIN_2 $pair_dom_res]>-1} {
			set is_intermediate 1
		} elseif {[lsearch $LIST_DOM_LIKE_BASIN_1 $pair_dom_res]>-1} {
			set is_intermediate 0
		} else {
			set is_intermediate 2
		}
	}
	lappend list_is_intermediate $is_intermediate
}
puts "set list_is_intermediate"

# collect contacts to mix and already handled contact according to basins
set list_pair_cont_basin_1_is_add {}
set list_pair_cont_basin_1_to_mix {}
set list_pair_cont_basin_2_is_add {}
set list_pair_cont_basin_2_to_mix {}
foreach is_intermediate $list_is_intermediate basin $list_basin res1 $list_res1 res2 $list_res2 {
	# create contact
	set pair_res1_res2 [list $res1 $res2]
	set pair_res1_res2 [lsort -integer $pair_res1_res2]
	if {$basin==1} {
		# basin 1
		if {$is_intermediate==2} {
			# to mix
			lappend list_pair_cont_basin_1_to_mix $pair_res1_res2
		} else {
			# already handled
			lappend list_pair_cont_basin_1_is_add $pair_res1_res2
		}
	} else {
		# basin 2
		if {$is_intermediate==2} {
			lappend list_pair_cont_basin_2_to_mix $pair_res1_res2
		} else {
			lappend list_pair_cont_basin_2_is_add $pair_res1_res2
		}
	}
}

# open pdb
set mol_basin_1 [mol new $PDB_NAME_BASIN_1]
set mol_basin_2 [mol new $PDB_NAME_BASIN_2]

# find centers of already-handled contacts
set list_pair_is_add_basin_1_center {}
set list_pair_is_add_basin_2_center {}
# basin 1
foreach pair_res1_res2 $list_pair_cont_basin_1_is_add {
	set pair_center [proc_center_geom_res_list $pair_res1_res2 $mol_basin_1]
	lappend list_pair_is_add_basin_1_center $pair_center
}
# basin 2
foreach pair_res1_res2 $list_pair_cont_basin_2_is_add {
	set pair_center [proc_center_geom_res_list $pair_res1_res2 $mol_basin_2]
	lappend list_pair_is_add_basin_2_center $pair_center
}
puts "set list_pair_is_add_basin_1_center"
puts "set list_pair_is_add_basin_2_center"


# for each mixed contact in each basin-find the closest contact
# basin 1
foreach pair_res1_res2 $list_pair_cont_basin_1_to_mix {
	set pair_center [proc_center_geom_res_list $pair_res1_res2 $mol_basin_1]
	# measure distances to all pairs in, find nearest pair
	set dist_min $DIST_MIN_CUTOFF_START
	foreach pair_ref $list_pair_cont_basin_1_is_add center_ref $list_pair_is_add_basin_1_center {
		set dist [proc_vec_dist $pair_center $center_ref]
		if {$dist<$dist_min} {
			set dist_min $dist
			set pair_ref_min $pair_ref
		}
	}
	# find add status of nearest pair
	set ind_ref_pair [lsearch $list_pair_sort $pair_ref_min]
	set is_intermediate [lindex $list_is_intermediate $ind_ref_pair]
	if {$is_intermediate==1} {
		# if nearest pair added to intermediate: add the current contact too
		set is_intermediate_current 1
	} else {
		# do not add current pair
		set is_intermediate_current 0
	}
	set ind_pair [lsearch $list_pair_sort $pair_res1_res2]
	set list_is_intermediate [lreplace $list_is_intermediate $ind_pair $ind_pair $is_intermediate_current]
}
# basin 2
foreach pair_res1_res2 $list_pair_cont_basin_2_to_mix {
	set pair_center [proc_center_geom_res_list $pair_res1_res2 $mol_basin_2]
	# measure distances to all pairs in, find nearest pair
	set dist_min $DIST_MIN_CUTOFF_START
	foreach pair_ref $list_pair_cont_basin_2_is_add center_ref $list_pair_is_add_basin_2_center {
		set dist [proc_vec_dist $pair_center $center_ref]
		if {$dist<$dist_min} {
			set dist_min $dist
			set pair_ref_min $pair_ref
...
# open output file
set grotop_new_end [proc_str_new_end $FILE_NAME_IN "txt"]
set file_name_out "patch_basin_${STR_TRIPBAS}_${grotop_new_end}"
set file_out [open $file_name_out w]

# create patch
foreach line $list_line is_intermediate $list_is_intermediate {
	if {$is_intermediate==1} {
		set line_change_basin [lreplace $line 3 3 $BASIN_NEW_NUM]
		set list_spaces [proc_str_list_spaces $line]
		set str_line_basin_new [proc_str_from_list_spaces $line_change_basin $list_spaces]
		puts $file_out $str_line_basin_new
	}
}
