#!/usr/bin/tclsh
# kmeans clustering of 2d hist with predefined centroids
# input: hist-2d
# input: LIST_CENTER: list of cluster centroids defined by (x,y) coor
# output: 
##############################################################################################################
set FILE_NAME_IN DoME_TB_4akea_1akea_0p6dc_2p6cr_lidmix_strong_all_4akea_13000_m61p32_0_m51p0_nmp_lid_2d_proc_hist.pmf
#set LIST_CENTER {{46 111} {65 111} {73 147}}
set LIST_CENTER {{46 111} {65 111} {73 147}}
set SCRIPT_NAME kmeans_clust_hist2d_predef
###############################################################################################################
# i/o
#####
...
puts $FILE_NAME_IN
puts ""
puts "predefined\t$LIST_CENTER"

# read data
set list_x [proc_list_clean_empty [proc_file_read_col_n $FILE_NAME_IN 0]]
set list_y [proc_list_clean_empty [proc_file_read_col_n $FILE_NAME_IN 1]]
set list_hist [proc_list_clean_empty [proc_file_read_col_n $FILE_NAME_IN 2]]
...
set list_center_initial {}
set ind_clust 0
foreach center $LIST_CENTER {
	# calc dist of first point
	set pt_1 [list [lindex $list_x 0] [lindex $list_y 0]]
	set dist_min [proc_vec_dist $center $pt_1]
	set x_nearest [lindex $list_x 0]
	set y_nearest [lindex $list_y 0]
	set ind_now 0
	foreach x $list_x y $list_y hist $list_hist {
		set dist_now [proc_vec_dist $center [list $x $y]]
		if {$dist_now<$dist_min} {
			set dist_min $dist_now
			set x_nearest $x
			set y_nearest $y
			set hist_nearest $hist
			set ind_nearest $ind_now
		}
		incr ind_now
	}
	set list_center [list $x_nearest $y_nearest $hist_nearest]
	lappend list_center_initial $list_center
	# append to new list
	lappend list_x_rev $x_nearest
	lappend list_y_rev $y_nearest
	lappend list_hist_rev $hist_nearest
	lappend list_clust_rev $ind_clust
	incr ind_clust
}
# remove nearest elements
set list_x_no_center [lreplace $list_x $ind_nearest $ind_nearest]
set list_y_no_center [lreplace $list_y $ind_nearest $ind_nearest]
set list_hist_no_center [lreplace $list_hist $ind_nearest $ind_nearest]
puts "initial\t$list_center_initial"

# initial assignment to cluster center
set list_which_clust {}
foreach x $list_x_no_center y $list_y_no_center hist $list_hist_no_center {
	# assign to cluster
	set list_dist_all_center {}
	foreach list_center $list_center_initial {
		set xy_center [lrange $list_center 0 1]
		set dist [proc_vec_dist $xy_center [list $x $y]]
		lappend list_dist_all_center $dist
	}
	set ind_min_clust [proc_list_get_ind_min_val $list_dist_all_center]
	set list_clust_near [lindex $list_center_initial $ind_min_clust]
	# calc new center of cluster
	set list_x_both [list $x [lindex $list_clust_near 0]]
	set list_y_both [list $y [lindex $list_clust_near 1]]
	set list_hist_both [list $hist [lindex $list_clust_near 2]]
	set list_x_new [proc_list_get_avg_weight $list_x_both $list_hist_both]
	set list_y_new [proc_list_get_avg_weight $list_y_both $list_hist_both]
	set list_hist_new [expr $hist+[lindex $list_clust_near 2]]
	set list_center_new [list $list_x_new $list_y_new $list_hist_new]
	# update value of nearest cluster
	set list_center_initial [lreplace $list_center_initial $ind_min_clust $ind_min_clust $list_center_new]
	# add to revised cluster list
	lappend list_x_rev $x
	lappend list_y_rev $y
	lappend list_hist_rev $hist
	lappend list_clust_rev $ind_min_clust
}
puts "after first assignment\t$list_center_initial"


# check each element again and relocate
set move 1
set step_num 1
while {$move==1} {
	puts "round\t$step_num\t$list_center_initial"
	set move 0
	for {set i 0} {$i<[llength $list_x_rev]} {incr i} {
		set x [lindex $list_x_rev $i]
		set y [lindex $list_y_rev $i]
		set hist [lindex $list_hist_rev $i]
		set num_clust [lindex $list_clust_rev $i]
		#puts "$x\t$y\t$hist\t$num_clust"
		...
		if {$num_clust!=$ind_min_clust} {
			set move 1
			# transfer to a new cluster
			# add to new cluster
			set list_nearest_clust [lindex $list_center_initial $ind_min_clust]
			set x_nearest_clust [lindex $list_nearest_clust 0]
			set y_nearest_clust [lindex $list_nearest_clust 1]
			set hist_nearest_clust [lindex $list_nearest_clust 2]
			set x_new [proc_list_get_avg_weight [list $x_nearest_clust $x] [list $hist_nearest_clust $hist]]
			set y_new [proc_list_get_avg_weight [list $y_nearest_clust $y] [list $hist_nearest_clust $hist]]
			set list_nearest_clust_add [list $x_new $y_new [expr $hist_nearest_clust+$hist]]
			# remove from old cluster
			set list_orig_clust [lindex $list_center_initial $num_clust]
			set x_orig [lindex $list_orig_clust 0]
			set y_orig [lindex $list_orig_clust 1]
			set hist_orig [lindex $list_orig_clust 2]
			set x_remove [proc_list_remove_from_weight_avg $x_orig $hist_orig $x $hist]
			set y_remove [proc_list_remove_from_weight_avg $y_orig $hist_orig $y $hist]
			set list_orig_clust_remove [list $x_remove $y_remove [expr $hist_orig-$hist]]
			# modify new and old clusters
			set list_center_initial [lreplace $list_center_initial $ind_min_clust $ind_min_clust $list_nearest_clust_add]
			set list_center_initial [lreplace $list_center_initial $num_clust $num_clust $list_orig_clust_remove]
			# assign to new cluster num
			set list_clust_rev [lreplace $list_clust_rev $i $i $ind_min_clust]
		}
	}
	incr step_num
}
puts "final\t$list_center_initial"
