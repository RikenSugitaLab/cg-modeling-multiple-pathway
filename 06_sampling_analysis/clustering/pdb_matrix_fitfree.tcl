#!/usr/bin/tclsh
#!/usr/local/bin/vmd
# calculates rmsd between all pairs of pdb files listed in files 
# input: pdb files and file with their names
# param: SEL_TXT: selection text for fitting
# output: rmsd between all pairs of pdb's
##############################################################################################################
set FILE_NAME_IN files.txt
set SEL_TXT "name CA"
set SCRIPT_NAME pdb_matrix_fitfree
###############################################################################################################
# i/o
#####
...
set file_name_out [proc_file_out_name_script_param1 $FILE_NAME_IN]
set file_out [open $file_name_out w]

for {set ind1 0} {$ind1<[llength $list_file_names]} {incr ind1} {
	set pdb1 [lindex $list_file_names $ind1]
	set mol1 [mol new $pdb1]
	set sel_fit1 [atomselect $mol1 $SEL_TXT]
	set sel_mv1 [atomselect $mol1 "all"]
	for {set ind2 $ind1} {$ind2<[llength $list_file_names]} {incr ind2} {
		# define mol/sel
		set pdb2 [lindex $list_file_names $ind2]
		set mol2 [mol new $pdb2]
		set sel_fit2 [atomselect $mol2 $SEL_TXT]
		set sel_mv2 [atomselect $mol2 "all"]
		# measure
		$sel_mv2 move [measure fit $sel_fit2 $sel_fit1]
		set rmsd12 [measure rmsd $sel_mv2 $sel_mv1]
		puts $file_out "$pdb1\t$pdb2\t$rmsd12"
		# end
		$sel_fit2 delete
		$sel_mv2 delete
		mol delete $mol2
