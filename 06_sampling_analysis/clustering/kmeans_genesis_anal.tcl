#!/usr/bin/tclsh
# summarizes and analyzes genesis kmeans clustering results
# input: list of .out files from kmeans clustering of genesis
# output: clusters sorted according to occupancy (high to low), cluster index, cluster center, cluster radius, #structures, percentage of this clisters, percentage of all accumulating clusters 
##############################################################################################################
set FILE_NAME_IN files.txt
set SCRIPT_NAME kmeans_genesis_anal
###############################################################################################################
# i/o
#####
...
# sorts a list according to indices
# input: list_target
# input: list_ind: sorted list of indices of elements of list_target
# output: list_target sorted according to the indices of list_ind (puts element of list_target according to the order of appearence of their list_ind)
proc proc_sort_list_indices {list_target list_ind} {
	set list_ans {}
	foreach ind $list_ind {
...
# returns only uncommon elements in a group of strings
# input: list_str: list with strings (all spearated by the same separator and with same length)
# input: sep_str: separator between different parts of string (a single separator for all)
# output: list for each string, in which parts of the string which are common to all are returned as sublists
proc proc_string_uncommon_multi {list_str sep_str} {
	# collect elements by location
	set list_list_str_el {}
	foreach str $list_str {
		set list_str_el [split $str ${sep_str}]
		lappend list_list_str_el $list_str_el
	}
	set list_list_el_str [proc_list_invert_dimension $list_list_str_el]
	# go over each element and decide whether to leave it or not
	set ind_el 0
	set list_ind_uncommon {}
	foreach list_el $list_list_el_str {
		set list_el_u [lsort -unique $list_el]
		if {[llength $list_el_u]>1} {
			lappend list_ind_uncommon $ind_el
		}
		incr ind_el
	}
	# build uncommon lists for each string
	set list_list_uncommon_str {}
	foreach list_str_sep $list_list_str_el {
		set list_str_uncommon [proc_lindex_all_list $list_str_sep $list_ind_uncommon]
		lappend list_list_uncommon_str $list_str_uncommon
	}
	return $list_list_uncommon_str
}

# joins a list to a string sperated by a given separator
# input: list_tar
# input: str_sep
# output: all elements of list_tar as a string, spearated by str_sep
proc proc_list_to_str_sep {list_tar str_sep} {
	set str_all ""
	set is_first 1
	foreach el $list_tar {
		if {$is_first==1} {
			set str_all "${str_all}${el}"
			set is_first 0
		} else {
			set str_all "${str_all}${str_sep}${el}"
		}
	}
	return $str_all
}

# returns only common elements of a list of strings
# input: list of strings, strings are separated by sep_str, all same length
# input: sep_str: the separator between strings elements
# input: val_uncommon: the string to put instead of uncommon parts
# output: only parts which are common in all strings, uncommon parts are replaced by val_uncommon
proc proc_string_common_multi {list_str sep_str val_uncommon} {
	set list_list_str_el {}
	foreach str $list_str {
		set list_sep [split $str "${sep_str}"]
		lappend list_list_str_el $list_sep
	}
	set list_list_el_str [proc_list_invert_dimension $list_list_str_el]
	# look for only common elements
	set list_str_common {}
	foreach list_el $list_list_el_str {
		set list_el_sort [lsort -unique $list_el]
		if {[llength $list_el_sort]==1} {
			lappend list_str_common [lindex $list_el 0]
		} else {
			lappend list_str_common $val_uncommon
		}
	}
	set str_common [proc_list_to_str_sep $list_str_common "${sep_str}"]
	return $str_common
}
...
# replaces a substring in a filename by a new string, string separated val_sep
# input: str_tar: string, separated by val_sep, with a "." file ending
# input: str_old, str_new
# input: val_sep: the separator between substrings
# output: the name of the file, with str_old replaced by str_new
proc proc_filename_replace_str_by_str_sep {str_tar str_old str_new val_sep} {
	set list_filename_split [split $str_tar "."]
	set str_filename [lindex $list_filename_split 0]
	set list_str_split [split $str_filename ${val_sep}]
	set ind_str_old [lsearch $list_str_split $str_old]
	set list_str_new [lreplace $list_str_split $ind_str_old $ind_str_old $str_new]
	set str_new_noend [proc_list_to_str_sep $list_str_new $val_sep]
	set str_new "${str_new_noend}.[lindex $list_filename_split 1]"
	return $str_new
}
...
# removes an element from list according to values
# input: list_tar
# input: val_del
# output: removes all val_del element from list
# removes an element from list
proc proc_list_remove_val {list_tar val_del} {
	set list_ans {}
	foreach el $list_tar {
		if {$el!=$val_del} {
			lappend list_ans $el
...
# separates a list into sublists of continuous element
# input: list of integers
# output: list of sublists, each sublist contains all continuous elements
proc proc_get_sublist_contin_int {list_target} {
	set list_list_sub_all {}
	set list_sub {}
	# first element
	set list_target_ends [lrange $list_target 1 end]
	lappend list_sub [lindex $list_target 0]
	foreach el $list_target_ends {
		set el_last [lindex $list_sub end]
		if {[expr $el-$el_last]>1} {
			# if not coninuous
			lappend list_list_sub_all $list_sub
			set list_sub {}
			lappend list_sub $el
		} else {
			# continuous
	...
	lappend list_list_sub_all $list_sub
	return $list_list_sub_all
}

# deletes all middle elements from a list
# input: list_target
# output: list_target with only end elements, if there is only one element, returns it twice as first and last element
proc proc_list_delete_middle {list_target} {
	set list_ans {}
	lappend list_ans [lindex $list_target 0]
	lappend list_ans [lindex $list_target end]
	return $list_ans
}

# divides a list of integers into sublists of ranges
# input: list_target: list of integers, increasing
# output: list of sublist where each sublist contains ranges of continuous elements
# example: {1 2 4 5 6 8 11 12} -> {{1 2} {4 6} {8} {11 12}}
proc proc_list_sublist_range_cont {list_target} {
	set list_ans {}
	set list_sublist_cont [proc_get_sublist_contin_int $list_target]
	foreach list_sub $list_sublist_cont {
		set list_range [proc_list_delete_middle $list_sub]
		lappend list_ans $list_range
	}
	return $list_ans
}
...
###############################################################################################################
# read fr files
set list_file_names [proc_file_read_to_list $FILE_NAME_IN]

# for each file
foreach file_name $list_file_names {
	# find last iteration number
	set list_anal [exec grep Analyze $file_name]
	set iter_last [lindex $list_anal end-15]
	# read data of last iteration
	set file_in [open $file_name r]
	set is_read 0
	# preapre lists for all properties
	set list_clust_ind {}
	set list_clust_center {}
	set list_clust_pop {}
	set list_clust_radius {}
	# read file
	while {[gets $file_in line]>=0} {
		set word_line_0 [lindex $line 0]
		set word_line_1 [lindex $line 1]
		# check if reached Analyze> directive
		if {$word_line_0=="Analyze>" && $word_line_1=="k-means"} {
			if {[lindex $line end]==$iter_last} {
				set is_read 1
				continue
			}
		}
		if {$is_read==1} {
			...
			if {[string is integer [lindex $line 0]]==1} {
				lappend list_clust_ind [lindex $line 0]
				lappend list_clust_center [lindex $line 1]
				lappend list_clust_pop [lindex $line 2]
				lappend list_clust_radius [lindex $line 3]
			}
		}
	}
	close $file_in
	# analyze clusters
	# sort all lists according to decreasing order
	set list_ind_pop_large [lsort -indices -decreasing -integer $list_clust_pop]
	set list_clust_pop_sort [proc_sort_list_indices $list_clust_pop $list_ind_pop_large]
	set list_clust_ind_sort [proc_sort_list_indices $list_clust_ind $list_ind_pop_large]
	set list_clust_center_sort [proc_sort_list_indices $list_clust_center $list_ind_pop_large]
	set list_clust_radius_sort [proc_sort_list_indices $list_clust_radius $list_ind_pop_large]
	# calc relative population of each cluster
	set list_pop_rel_sort {}
	set sum_pop [proc_list_get_sum_elem $list_clust_pop_sort]
	foreach pop $list_clust_pop_sort {
		set pop_rel [expr $pop*1.0/$sum_pop]
		lappend list_pop_rel_sort $pop_rel
	}
	# calc accummulated population after each cluster
	set list_pop_accum {}
	set pop_sum_accum 0
	foreach pop $list_clust_pop_sort {
		set pop_sum_accum [expr $pop_sum_accum+$pop]
		set pop_accum_rel [expr $pop_sum_accum*1.0/$sum_pop]
		lappend list_pop_accum $pop_accum_rel
	}
	# write results
	set file_name_in_no_path [proc_file_name_no_path $file_name]
	set file_name_out [proc_file_name_insert_str $file_name_in_no_path "stats"]
	set file_out [open $file_name_out w]
	puts $file_out "ind\tcenter\tradius\tpopulation\tpopulation-rel\tpopulation-accum"
	foreach ind $list_clust_ind_sort center $list_clust_center_sort radius $list_clust_radius_sort pop $list_clust_pop_sort pop_rel $list_pop_rel_sort pop_accum $list_pop_accum {
		puts $file_out "$ind\t$center\t$radius\t$pop\t$pop_rel\t$pop_accum"
	}
	close $file_out
}
