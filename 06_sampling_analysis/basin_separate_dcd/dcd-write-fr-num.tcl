#!/usr/local/bin/vmd
#!/usr/bin/tclsh
# writes specified fr from a dcd file
# input: dcd, pdb, psf files 
# input: files with frame numbers (multiple files possible)
# ouptut: dcd file with only the specified frames written 
##############################################################################################################
set FILE_NAME_IN files_fr.txt
set DCD_NAME ../data/dcd/DoME_TB_4akea_1akea_0p6dc_2p6cr_nmpmix_4akea_10850_m59p5_0_m13p8-74-75-77.dcd
set PDB_NAME GO_1akea.pdb
set PSF_NAME GO_1akea.psf
set SCRIPT_NAME dcd-write-fr-num
###############################################################################################################
# i/o
#####
...
set list_file_names_no_path_end {}
foreach file_name $list_file_names {
	set file_name_no_path [proc_file_name_no_path $file_name]
	set file_name_noend [proc_str_no_end $file_name_no_path]
	lappend list_file_names_no_path_end $file_name_noend
}
set list_name_u [proc_string_uncommon_multi $list_file_names_no_path_end "_"]
...
# for each file
foreach file_name $list_file_names name_u $list_name_u {
	# set dcd file name
	set file_dcd_no_path [proc_file_name_no_path $DCD_NAME]
	set file_dcd_name [proc_file_name_insert_str $file_dcd_no_path $name_u]
	# read frames
	# create ranges of continuous frames
	set list_fr [proc_file_read_to_list $file_name]
	set list_fr_range [proc_list_sublist_range_cont $list_fr]
	# write each fr
	foreach fr_range $list_fr_range {
		puts $fr_range
		set fr_start [lindex $fr_range 0]
		set fr_end [lindex $fr_range 1]
		exec catdcd -o temp_${fr_start}_${fr_end}_${name_u}.dcd -first $fr_start -last $fr_end $DCD_NAME
	}
	exec /bin/sh -c "catdcd -o $file_dcd_name temp*_${name_u}.dcd"
}
