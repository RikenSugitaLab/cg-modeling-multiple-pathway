#!/usr/bin/tclsh
# returns fr within an encompassed aread in a 2D pmf
# input: key-val1-val2
# param: LIST_BORDER: min/max ranges for val1/val2
# output: frames which have val1, val2 within the specified range (ignoring the original key)
##############################################################################################################
set FILE_NAME_IN files.txt
set LIST_BORDER {{30 65} {123 200}}
set NUM_BASIN 3
set SCRIPT_NAME pmf-2d-divide-fr
###############################################################################################################
# i/o
#####
...
###############################################################################################################
# set borders 
set val1_min [lindex [lindex $LIST_BORDER 0] 0]
set val1_max [lindex [lindex $LIST_BORDER 0] 1]
set val2_min [lindex [lindex $LIST_BORDER 1] 0]
set val2_max [lindex [lindex $LIST_BORDER 1] 1]

# read files
set list_file_name [proc_file_read_to_list $FILE_NAME_IN]


# go over each file
set fr_now 1
foreach file_name $list_file_name {
	# prepare data
	set list_val1 [proc_file_read_col_n $file_name 1]
	set list_val2 [proc_file_read_col_n $file_name 2]
	# output file
	set file_name_out [proc_file_name_insert_str $file_name "basin_${NUM_BASIN}"]
	set file_out [open $file_name_out w]
	# go over each fr
	foreach val1 $list_val1 val2 $list_val2 {
		if {(($val1>$val1_min) && ($val1<$val1_max)) && (($val2>$val2_min) && ($val2<$val2_max))} {
			puts $file_out $fr_now
		}
		incr fr_now
	}
	close $file_out
