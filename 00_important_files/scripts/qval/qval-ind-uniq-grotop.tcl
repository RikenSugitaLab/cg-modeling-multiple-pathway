#!/usr/bin/tclsh
# calculates qval-uniq over dcd for each basin
# input: dcd, pdb, psf files (multi dcd)
# input: grotop file
# param: DIST_CUTOFF_CONT: cutoff for native contact (xtimes the original length)
# output: for each basin, a file with list of contacts and its qval along the traj
##############################################################################################################
set FILE_NAME_IN files.txt
set PDB_NAME GO_1akea.pdb
set PSF_NAME GO_1akea.psf
set FILE_NAME_GROTOP DoME_TB_4akea_1akea_0p6dc_2p6cr_nmpmix.grotop
set DIST_CUTOFF_CONT 1.2
set SCRIPT_NAME qval-ind-uniq-grotop
###############################################################################################################
# i/o
#####
...
# returns all elements equal or smaller than x
# input: list_tar 
# input: cuoff_el
# output: list of all elements equal or smaller than cutoff_el 
proc proc_list_get_le_cutoff {list_tar cutoff_el} {
	set list_ans {}
	foreach el $list_tar {
		if {$el<=$cutoff_el} {
			lappend list_ans $el
...
###############################################################################################################
# process grotop file
set file_in [open $FILE_NAME_GROTOP r]
set is_read 0
set list_pair_cont {}
set list_basin {}
set list_dist {}
while {[gets $file_in line]>=0} {
	if {[string match *\\\[* $line]==1} {
		if {[string match *multicontact* $line]==1} {
			set is_read 1
			continue
		}
	}
	...
	if {$is_read==1} {
		# read unique contacts
		lappend list_pair_cont [list [lindex $line 0] [lindex $line 1]]
		lappend list_basin [lindex $line 3]
		lappend list_dist [expr [lindex $line 4]*10]
	}
}
close $file_in

# divide contacts intp basins
set list_basin_uniq [lsort -unique $list_basin]
set list_list_basin_pair_contact {}
set list_list_basin_dist {}
foreach basin_u $list_basin_uniq {
	set list_ind_basin [lsearch -all $list_basin $basin_u]
	set list_pair_ind_basin [proc_lindex_all_list $list_pair_cont $list_ind_basin]
	set list_dist_ind_basin [proc_lindex_all_list $list_dist $list_ind_basin]
	lappend list_list_basin_pair_contact $list_pair_ind_basin
	lappend list_list_basin_dist $list_dist_ind_basin
}

# open each dcd
set list_file_names [proc_file_read_to_list $FILE_NAME_IN]
foreach file_name $list_file_names {
	set mol_now [proc_dcd_open $file_name $PSF_NAME $PDB_NAME]
	# for each basin
	foreach basin_u $list_basin_uniq list_pair_cont $list_list_basin_pair_contact list_dist $list_list_basin_dist {
		# set output file
		set dcd_no_path [proc_file_name_no_path $file_name]
		set dcd_noend [proc_str_no_end $dcd_no_path]
		set file_name_out [proc_file_out_name_script_param2 $dcd_noend "basin${basin_u}"]
		set file_out [open $file_name_out w]
		# foreach contact
		foreach pair_cont $list_pair_cont dist_orig $list_dist {
			# get all distances
			set res1 [lindex $pair_cont 0]
			set res2 [lindex $pair_cont 1]
			set indatm1 [proc_resid_to_ca_index $res1 $PDB_NAME]
			set indatm2 [proc_resid_to_ca_index $res2 $PDB_NAME]
			set list_dist_now [measure bond [list $indatm1 $indatm2] molid $mol_now frame all]
			# calc qval
			set num_fr [llength $list_dist_now]
			set dist_orig_buff [expr $dist_orig*$DIST_CUTOFF_CONT]
			set list_dist_le_cutoff [proc_list_get_le_cutoff $list_dist_now $dist_orig_buff]
			set qval_cont [expr [llength $list_dist_le_cutoff]*1.0/$num_fr]
			# write
			puts $file_out "$pair_cont\t$qval_cont"
		}
		close $file_out
	}
	mol delete $mol_now
