#!/usr/bin/tclsh
# converts a 2d pmf into 2 2d hist
# input: x-y-pmf (blank line between each x)
# param: TEMP
# output: x-y-hist (normalized)
##############################################################################################################
set FILE_NAME_IN ../DoME_TB_4akea_1akea_0p6dc_2p6cr_lidmix_xxx_12000_xxx_0_m50p5_ene_target_14000_m60p86_0_m49p7_2d_proc.pmf
set TEMP 200
set KB 0.001987191
set ELOG 2.71828188
set SCRIPT_NAME pmf_to_hist_2d
###############################################################################################################
# i/o
#####
...
set list_pmf [proc_file_read_col_n $FILE_NAME_IN 2]
# remove empty lines
set list_x [proc_list_clean_empty $list_x]
set list_y [proc_list_clean_empty $list_y]
set list_pmf [proc_list_clean_empty $list_pmf]

# convert each pmf
set list_hist {}
foreach pmf $list_pmf {
	set prob [expr $ELOG**(-1.0*$pmf/($KB*$TEMP))]
	lappend list_hist $prob
}
set list_hist_norm [proc_list_get_normalized $list_hist]

# write
set file_name_no_path [proc_file_name_no_path $FILE_NAME_IN]
set file_name_out [proc_file_name_insert_str $file_name_no_path "hist"]
set file_out [open $file_name_out w]
set x_prev [lindex $list_x 0]
foreach x $list_x y $list_y hist_norm $list_hist_norm {
	if {$x!=$x_prev} {
		puts $file_out ""
	}
	puts $file_out "$x\t$y\t$hist_norm"
	set x_prev $x
}
