#!/usr/bin/tclsh
# edits a genesis-output 2d pmf file to a gnuplot file format
# input: FILE_NAME_PMF: genesis 2d pmf output file (pmf at each xy bin)
# input: FILE_NAME_LOG: log file from pmf calculation, contains bins
# output: x,y,pmf ("Infinity" values take the maximum value*MAX_PMF_RATIO), there is a space between each different x
# comment: there is a bug in the genesis pmf_analysis which inverts xy axes, this script takes this into account
##############################################################################################################
set FILE_NAME_PMF DoME_TB_4akea_1akea_0p6dc_2p6cr_lidmix_xxx_12000_xxx_0_m50p5_ene_target_14000_m60p86_0_m49p7_2d.pmf
set FILE_NAME_LOG pmf2d-input_exec_14000_m60p86_0_m49p7.out
set MAX_PMF_RATIO 1
set MAX_DEFAULT_VAL -100
set SCRIPT_NAME pmf-2d-genesis-proc
###############################################################################################################
# i/o
#####
...
###############################################################################################################
# read grids from log file
set list_line_setup_option [exec grep -n Setup_Option $FILE_NAME_LOG]
set str_setup_option_1 [lindex $list_line_setup_option 0]
set str_setup_option_2 [lindex $list_line_setup_option 7]
set list_line_setup_option_1 [split $str_setup_option_1 :]
set list_line_setup_option_2 [split $str_setup_option_2 :]
set line_setup_option_1 [lindex $list_line_setup_option_1 0]
set line_setup_option_2 [lindex $list_line_setup_option_2 0]
set line_grids_1 [expr $line_setup_option_1+1]
set line_grids_2 [expr $line_setup_option_2+1]
set list_line_grids_1 [exec sed "${line_grids_1}q;d" $FILE_NAME_LOG]
set list_line_grids_2 [exec sed "${line_grids_2}q;d" $FILE_NAME_LOG]

# read .pmf file
set list_list_y_x_pmf [proc_file_2d_read_to_list $FILE_NAME_PMF]
set list_list_x_y_pmf [proc_list_invert_dimension $list_list_y_x_pmf]

# find max val
set max_now $MAX_DEFAULT_VAL
set list_x_y_pmf_all [join $list_list_x_y_pmf]
foreach el $list_x_y_pmf_all {
	if {$el>$max_now && $el!="Infinity"} {
		set max_now $el
	}
}
set max_pmf [expr $max_now*$MAX_PMF_RATIO]

# find min val
set min_now $max_now
foreach el $list_x_y_pmf_all {
	if {$el<$min_now && $el!="Infinity"} {
		set min_now $el
	}
}
set min_val $min_now
...
# output file
set file_name_out [proc_file_name_insert_str $FILE_NAME_PMF "proc"]
set file_out [open $file_name_out w]

foreach grid1 $list_line_grids_1 list_pmf $list_list_x_y_pmf {
	foreach grid2 $list_line_grids_2 pmf_val $list_pmf {
		if {$pmf_val=="Infinity"} {
			set pmf_val $max_pmf
		}
		set pmf_val [expr $pmf_val-$min_val]
		puts $file_out "$grid1\t$grid2\t$pmf_val"
	}
	puts $file_out ""
}
